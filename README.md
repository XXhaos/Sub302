...
- 不解析、不合并、不转换订阅内容
- 本地 Sub-Store 或 代理软件 负责订阅拉取、转换和聚合处理，避免cf ip或者海外ip被机场封禁导致的无法拉取订阅
...
- 我的订阅：创建订阅组，支持“引用清单”或“302 到聚合地址”；引用清单默认 Base64 输出
- 设置：公开基址、控制台安全路径、订阅 Token、重定向状态码、订阅组前缀、数据备份
- 主题：控制台支持深色、浅色和跟随系统
...
- 引用清单：`/p/组slug` 默认返回 Base64 文本，解码后内容是选中的 Sub302 固定入口和手动节点 URI
