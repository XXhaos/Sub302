...
> 注意：这里的 `Deploy command` 是 Cloudflare 构建环境内部执行的命令，不是让你在本地手动部署。Worker 名称建议保持为 `sub302-cf`，需要和 `wrangler.toml` 里的 `name` 一致。
...
部署完成后进入你的 Worker 项目设置：

1. 打开 `Settings`
2. 进入 `Bindings`
3. 在 `KV namespace bindings` 中点击 `Add binding`
4. Variable name 填写：`SUB_ROUTES`
5. KV namespace 选择已有命名空间，或新建一个命名空间
6. 保存后重新部署一次项目
...
| 变量名 | 类型 | 说明 | 示例 |
| --- | --- | --- | --- |
| `ADMIN_PASSWORD` | Secret | 后台登录密码 | `your_secure_password` |

可选变量：

| 变量名 | 类型 | 说明 | 默认值 |
| --- | --- | --- | --- |
| `REDIRECT_STATUS_CODE` | Variable | 默认重定向状态码 | `302` |

`ADMIN_PASSWORD` 必须设置，否则后台无法登录。

### 3. 重新部署

KV 和环境变量保存后，在 Cloudflare 的 `Deployments` 页面重新部署最新版本，或向 GitHub 推送一个新提交触发自动部署。
...
- `SUB_ROUTES`：Cloudflare KV namespace binding，必需
- `ADMIN_PASSWORD`：后台管理密码，必需，建议使用 Secret
- `REDIRECT_STATUS_CODE`：默认重定向状态码，默认 `302`
