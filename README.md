...
4. KV 名称固定建议使用：`sub302`
5. 创建后复制这个 KV namespace 的 `Namespace ID`
6. 打开仓库里的 `wrangler.toml`
7. 把下面这一行替换成你的 KV namespace id：

```toml
id = "REPLACE_WITH_YOUR_SUB302_KV_NAMESPACE_ID"
```

固定配置已经写在 `wrangler.toml` 里，不要改 `binding` 名：
...
也就是说，Sub302 统一只认这个 KV binding：

```text
SUB302_KV
```

改完后提交并推送到 GitHub，Cloudflare 会自动重新部署。
...
- `SUB302_KV`：固定的 Cloudflare KV namespace binding，必需
